...
//char * comports[MAXRUNNINGPORT][MAX_NUM_NAME] = {0,};
int _lastindex = 0;
map<int, char * > comports;
int FindValue(const char * value)
{
	bool found = false;
	map<int, char*>::iterator it = comports.begin(); // internalMap is std::map
	while (it != comports.end())
	{
		if(!strcmp(it->second,value));
		return it->first;
		++it;
	}
	return -1;
...
#ifndef __PPC__
char * comports[]={"/dev/ttyS0","/dev/ttyS1","/dev/ttyS2","/dev/ttyS3","/dev/ttyS4","/dev/ttyS5",
...
#else
char * comports[]={"/dev/ttyPSC0","/dev/ttyPSC1","/dev/ttyPSC2","/dev/ttyPSC3","/dev/ttyPSC4","/dev/ttyPSC5","/dev/ttyPSC6","/dev/ttyPSC7","/dev/ttyPSC8","/dev/ttyPSC9","ttyPSC5",NULL};
...
int RS232_OpenComport(int comport_number, int baudrate, const char *mode)
{
  ...
  if((comport_number>=_lastindex)||(comport_number<0))
  {
...
HANDLE Cport[MAXRUNNINGPORT];

#if 0
char * comports[]={"\\\\.\\COM1",  "\\\\.\\COM2",  "\\\\.\\COM3",  "\\\\.\\COM4",
                       "\\\\.\\COM5",  "\\\\.\\COM6",  "\\\\.\\COM7",  "\\\\.\\COM8",
