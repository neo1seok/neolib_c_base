...
{	
    ...
    {
        if ((DWORD)(str[i] - '0') <= ('9' - '0'))
            dwRet += (DWORD)(str[i] - '0');
        else if ((DWORD)(str[i] - 'A') <= ('F' - 'A'))
            dwRet += (DWORD)(str[i] + 10 - 'A');
        else if ((DWORD)(str[i] - 'a') <= ('f' - 'a'))
            dwRet += (DWORD)(str[i] + 10 - 'a');
        else
            continue;
...
string GetSHA256Hexstr(BYTE *org, int orgSize)
{
	// SDATA	32byte
	// ID		16byte
	// PIN		16byte
	//INFORM	16byte (Temp)
	BYTE SDATA[SHA256_DIGEST_LENGTH]
...
int Bn2Bin(const BIGNUM * pbignum,BYTE *pbuff,int bytesize)
{
...
DWORD calcCRC(BYTE* data, int iLength)
{
    ...
    for (i = 0; i < iLength; i++)
         crc = (crc >> 8) ^ CRC16TAB[(BYTE)(crc ^ data[i])];

//	crc = crc << 8 | crc >> 8 ; //++ swap because HW uart crc module

    return (DWORD)(unsigned short)crc;
}
